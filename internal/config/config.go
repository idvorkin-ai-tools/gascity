...
//
// Callers append their own bd flags (--limit=1 for the first-row work_query;
...
// poolDemandFirstRowProbe emits the work_query Tier 3 body for target: it reads
// the first ready, unassigned, routed bead, prints it, and exits 0. The caller
// appends a terminal fallthrough (printf "[]") for the empty case.
func poolDemandFirstRowProbe(target string) string {
	return `r=$(` + bdReadyPoolDemandShell(target) + ` --limit=1 2>/dev/null); ` +
...
// bdReadyPoolDemandShell with poolDemandFirstRowProbe so the reconciler's spawn
// decision and the worker's claim decision read the same predicate.
//
...
func (a *Agent) EffectiveWorkQuery() string {
	...
	if legacyTarget == "" {
		return `sh -c '` +
			...
			`[ -n "$r" ] && [ "$r" != "[]" ] && printf "%s" "$r" && exit 0; ` +
			`done; ` +
			// Tier 3: ready unassigned routed to this config via gc.routed_to
			// (shared routed queue).
			...
			`*) exit 0 ;; ` +
			`esac; ` +
			poolDemandFirstRowProbe(target) +
			`printf "[]"'`
	}
	return `sh -c '` +
		// Tier 1: in_progress assigned to any of my identifiers (crash recovery).
		...
		`done; ` +
		// Tier 3: ready unassigned routed to this config via gc.routed_to
		// (shared routed queue), then the legacy workflow-control route for
		// pre-rename graphs.
		// Only ephemeral sessions and controller probes consume generic config demand.
		`case "$GC_SESSION_ORIGIN" in ` +
		`ephemeral|"") ;; ` +
		...
		`esac; ` +
		poolDemandFirstRowProbe(target) +
		poolDemandFirstRowProbe(legacyTarget) +
		`printf "[]"'`
}
