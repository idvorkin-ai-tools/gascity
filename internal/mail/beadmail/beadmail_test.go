...
func TestInboxUsesSingleMessageScanAcrossRoutes(t *testing.T) {
	...
	if query.TierMode != beads.TierBoth || !query.AllowScan || query.Type != "message" || query.Status != "open" || query.Assignee != "" {
		t.Fatalf("message query = %+v, want one cross-tier message scan without per-route assignee", query)
	}
}
...
func TestCheckUsesBothTiersForSlashRecipient(t *testing.T) {
	...
	runner := func(_ string, name string, args ...string) ([]byte, error) {
		...
		case strings.Contains(cmd, "bd query --json"):
			if !strings.Contains(cmd, "ephemeral=true") || !strings.Contains(cmd, "type=message") || !strings.Contains(cmd, "status=open") {
				t.Fatalf("slash recipient used unexpected wisp query: %s", cmd)
			}
			if strings.Contains(cmd, "assignee="+recipient) {
				t.Fatalf("slash recipient used unsafe assignee clause: %s", cmd)
			}
			return []byte(`[{"id":"msg-1","title":"hello","description":"body","status":"open","issue_type":"message","assignee":"gascity/workflows.codex-max","from":"human","created_at":"2026-01-02T03:04:05Z","ephemeral":true}]`), nil
		}
		return nil, errors.New("unexpected command: " + cmd)
	}
	p := New(beads.NewBdStore(t.TempDir(), runner))
...
func TestMessageQueriesUseBothTiers(t *testing.T) {
	...
	if len(inbox) != 2 || !hasMailMessageID(inbox, msg.ID) || !hasMailMessageID(inbox, wisp.ID) {
		t.Fatalf("Check = %#v, want issues-tier message %s and wisp message %s", inbox, msg.ID, wisp.ID)
	}
	...
	if len(all) != 2 || !hasMailMessageID(all, msg.ID) || !hasMailMessageID(all, wisp.ID) {
		t.Fatalf("All = %#v, want issues-tier message %s and wisp message %s", all, msg.ID, wisp.ID)
	}
